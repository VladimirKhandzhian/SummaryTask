...
public class SendRequest extends Command{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String textOfRequest = request.getParameter("textOfRequest");
		LOGGER.debug("Request parameter: textOfRequest --> " + textOfRequest);
		
		int workerId = (int) session.getAttribute("workerId");
		LOGGER.debug("Get attribute: workerId --> " + workerId);		
			
		String forward = Page.DISPATCHER_CABINET_PAGE;
		
		Requests requests = new Requests();
		requests.setTextOfRequest(textOfRequest);
		requests.setIdWorker(workerId);
		
		daoInterfaceRequests.insertRequest(requests);
		...
		request.setAttribute("requests", daoInterfaceRequests.getRequestsByWorkerId(workerId));
		LOGGER.debug("Set attribute requests --> " + daoInterfaceRequests.getRequestsByWorkerId(workerId));
